...
\section{Conclusion}
As an extension study of homogeneous model~\cite{yoo_hydrodynamic_2020}, we have developed a numerical method to simulate a settling marine aggregate model, randomly formed with cubes, using a boundary integral method in a density-stratified fluid.
Applying the net force equilibrium in a Stokes regime, we prescribe the total drag and torque to solve for the stress vectors on the aggregate and its settling velocity using the single-layer potential formula.
With the velocity field obtained, we advance the perturbation in time by coupling the advection-diffusion equation. 
...
\par
We have validated our methods by providing results of a settling aggregate composed of 10 cubes while varying numerical grid and time steps and domain sizes. 
Most of the errors for each comparison appeared to be much smaller than the largest error we estimated is about 23\% from the stress value on each square face of a cube.
\par
Furthermore, we simulated larger aggregates, made with 50 cubes, in different settings of fluids as numerical results. We have observed aggregate settling behavior before its density becomes neutral to surrounding fluid density.
We find a somewhat consistent trend in the settling behaviors between different shapes of aggregates.   
We can certainly simulate aggregates with more cubes to explore many more different shapes. It would allow us to have better statistical results. 
Under the different Péclet number environments, we find the main changes in the perturbation, as expected, between Pe = 1 and Pe = 10. Since a smaller Péclet number implies a higher diffusivity, we observe the perturbation near the aggregate slows down 10\% faster than the other two cases. 
\par
We also explore various stratification strengths via $\gamma$ values. 
As we mentioned, marine aggregates are highly porous and sensitive to surrounding fluid density stratification~\cite{prairie_delayed_2013}. Our results also support these characteristics, while we perform the simulations with three types of background fluid density gradients.
In addition to our results shown in this thesis, potential future work includes considering another applicable regime with different parameters.
\par
