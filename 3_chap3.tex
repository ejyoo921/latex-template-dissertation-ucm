...
\end{equation}
By choosing $\vec{q} = \rho_0 \alpha \vec{g}$, we can find a articular solution to our modified Stokes momentum equation, (\ref{eq_extra_C}),
\begin{equation}
	\vec{u} (\vec{y}) =
...
First, we know that the total drag is sum of stress vectors, $\vec{f}$,
\begin{equation}
...
For the scale of the perturbation, $C$, we introduce the maximum density difference of the background density profile in the fluid domain at the initial time, i.e., {\color{blue} WHAT IS THE UNIT OF $C$? -> NOT THE SAME AS $\rho$}
\[
C_{max} = 
\left|
...
Note that we discussed the Péclet number in Chapter (\ref{sec:concentration}), while introducing its definition in equation (\ref{eq_def_Pe}).
In addition to the diffusion effect due to salinity, one may consider the thermal diffusivity of the ocean, $D_{heat}$~\cite{nayar_thermophysical_2016,sharqawy_thermophysical_2010}: we get a Péclet number of
\begin{align}
	\text{Pe} 
...
\begin{figure}[ht]
	\begin{center}
		\includegraphics[scale=0.35]{./figures/fig_sample_agg10.pdf}
