...
%validation---------------------------------------------------
\section{Validation}
In order to determine our simulation settings, we examine the simplified results by varying 1) time step $\Delta t$, 2) space step $\Delta x$, and 3) domain sizes. Note that we use an uniform spacing for all directions, i.e., $\Delta x = \Delta y = \Delta z$. To differ the fluid domain size, we consider the center of mass,  $\vec{x}_{cm}$, and the maximum radius of the aggregate, $R_m$.
