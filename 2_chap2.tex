% {\color{red} This chapter is referred by Yoo, Blanchette, and Khatri's own publication \cite{yoo_hydrodynamic_2020}. }
The text of This chapter has a reprint of our article that is published.~\fullcite{yoo_hydrodynamic_2020}.
%
\par
\vphantom{text}
\par
...
As marine aggregates have fractal structures, it is important to find an appropriate length scale to be able to predict their settling speed more accurately.
We thus consider a homogeneous fluid in this chapter to choose an effective hydrodynamic radius. 
...
We consider aggregates made of collated cubic particles, as shown in Figure~\ref{fig_rad_mass}. Once formed, we assume that aggregates do not deform, sinter, or break apart~\cite{eggersdorfer_multiparticle_2011}.
...
Typically, the IAA method gives a more compact form of aggregates, with dimension $\approx 2.56$, than the CCA case, which has dimension $\approx 1.79$~\cite{witten_diffusion-limited_1981, kaye_random_2008}. More details regarding the analysis of the aggregation formation can be found~\cite{yoo_hydrodynamic_2020}.
...
\label{eq_Rg}
\end{equation}
and the maximum radius of the aggregate, $R'_m$, as the
maximum distance between the center of each of its constituting cubes, $\vec{x}_i$, and the center of
...
We obtain the center of mass of the aggregate, $\vec{x}_{cm}$, by taking the mean values of each element of $\vec{x}_i$ for $i = 1, \cdots, NC$.
...
We will later show the results of different forces acting on the aggregates and examine which size measurement captures better physical properties.

%
%--------------------------------------------------------
\section{Derivation of single- and double-layer potentials}
We introduced equation~(\ref{eq_BIE_onS}) as a solution to the Stokes equations in Chapter~\ref{ch:intro}. As we have a solid boundary surface of a marine aggregate model, we are able to use a simplified version of the fundamental solution. We are interested in solving for the flow at points $\vec{y}$ in the fluid domain, which extends to infinity, and external to the surface, $S$, of an aggregate, as shown in Figure~\ref{fig_cube10}.
...
The resulting Fredholm integral formulas have the advantage of involving computations in two dimensions only, despite the three-dimensionality of the system.
For solid objects, one may obtain two different  Fredholm integral formulas. We derive both formulations in the remainder of this section, following the guidance of~\cite{pozrikidis_boundary_1992}. 
\subsection{Single-layer potential (SLP)}
We first derive the velocity with the single-layer potential alone.
The solid boundary condition allows us to express the aggregate's velocity as 
\begin{equation}
...
Substituting the solid object velocity, equation~(\ref{eq_solidbody}), into the double-layer potential in equation~(\ref{eq_BIE}) gives
\begin{align}
	&\int_S
...
\end{align}
\par
The first integral on the right-hand side of equation~(\ref{eq_solid_motion_dlp}) can be re-written as 
\begin{equation}
	\vec{U}_a \cdot
	\int_S
...
For the flow around (outside) and on the solid object, Pozrikidis provides a useful identity~\cite{pozrikidis_boundary_1992},
\begin{align}
...
\begin{align}
	...
	\label{eq_dlp_Ua}
\end{align}
% Applying these values to the representation formulas, equation(\ref{eq_BIE}) and~(\ref{eq_BIE_onS}), allows us to remove the double-layer potential.
\par 
...
\begin{align}
	&
	\varepsilon_{i \ell m}
	 \int_S x_{\ell} \ K_{mjk}(\vec{x},\vec{y})  \ n_{k} ( \vec{x})
	 \ \text{d}S(\vec{x})
	\nonumber \\
	 & = 
	 \begin{cases}
...
\begin{align}
	...
	\ \text{d}S(\vec{x})
	 = 
	\int_S 
	\left(  \varepsilon_{i \ell m} \ \Omega_{i} x_{\ell} \right) \ 	K_{mjk}(\vec{x},\vec{y})  \ n_{k} ( \vec{x})
	\ \text{d}S(\vec{x})
	\nonumber \\
	\nonumber \\
 	& = 
	...
	\label{eq_dlp_Omega}
\end{align}
We now combine the two equations,~(\ref{eq_dlp_Ua}) and~(\ref{eq_dlp_Omega}),
\begin{equation}
...
To analyze the force and torque, we use the stress $\vec{f}$ from equation~(\ref{eq_slp}) to compute
\begin{equation}
	\vec{F}_o= \int_s \vec{f}(\vec{x}) \  \text{d}S(\vec{x}) \hspace{1 cm} \text{ and } \hspace{1 cm}   \vec{Q}_o= \int_s (\vec{x} - \vec{x}_{cm}) \times \vec{f}(\vec{x})  \ \text{d}S(\vec{x}).
	\label{eq_total_FT}
\end{equation}
...
Note that the fluid volume $V$ is strictly outside the aggregate boundary $S$. 
\begin{figure}[h]
	\begin{center}
...
\end{center}
\end{figure}
The control volume could be a large sphere with a radius, $r_f$. Then the velocity $\vec{u}$ on $S_f$ approaches zero with order $\mathcal{O}(1/r_f)$ and its associated stress tensor decays at the rate $\mathcal{O} (1/r_f^2)$. 
For large enough $r_f$, the surface integral on $S_f$ in equation~(\ref{eq_div}) is zero, implying that
\[
\int_{S} \vec{u}(\vec{x}) \cdot \hat{n} \ \text{d}S(\vec{x}) = 0.
\]
%  It also means that the net force on the surface $S$ is zero. 
Then, following~\cite{pozrikidis_boundary_1992}, we have a complementary flow $\vec{u}_c$.  By the Lorentz reciprocal identity, any two flows having the same force on the boundary satisfy
...
When $\bar{\bar{K }}$ is a Green's function for an infinite flow, the velocity~(\ref{eq_BIE_for_dlp}) satisfies the Stokes equation with any choice of $\vec{\psi}(\vec{x}) $.
\par
 % Thus, the velocity formula with only the double-layer potential would be valid for the zero net force model.
Due to our constraints to derive equation~(\ref{eq_BIE_for_dlp}), the double-layer potential formula, $\vec{u}_{dl}$ is only valid for a particle that has a force- and torque-free body, such as a swimming microorganism.
...
\begin{equation}
 ...
 \end{equation}
%
We multiply by $\tilde{\mu} / L$ to have the same physical meaning when the stress vector, $\vec{f}$, as we found the forces using the single-layer potential, equation~(\ref{eq_total_FT}).
...
The flow $\vec{u}_F$ is the velocity at a point $\vec{y}$   generated by a point force with strength $\vec{F}_0$ at $\vec{x}_{cm}$. 
...
\vec{u}(\vec{y}) = 
&\frac{1}{8 \pi}\int_S
\vec{\psi}(\vec{x}) \cdot  \bar{\bar{K}}(\vec{x},\vec{y}) \cdot \hat{n}(\vec{x}) \ \text{d}S(\vec{x}) 
\nonumber \\
& + \frac{1}{8 \pi \tilde{\mu} }\vec{F}_o \cdot \bar{\bar{G}}(\vec{x}_{0},\vec{x}_{cm})
...
% \end{equation*}
for any points $\vec{y}$ in the fluid and strictly outside of the solid particle surface $S$.
...
&+\frac{1}{8\pi} \int_S  \vec{\psi}(\vec{x})  \cdot \bar{\bar{K}} (\vec{x},\vec{x}_s)  \cdot \hat{n}(\vec{x}) \ \text{d}S(\vec{x}) 
\nonumber \\
...
\par
equations for the non-dimensional force and angular velocity, given dimensionally in equation~(\ref{eq_total_FT}), become
\begin{align}
\vec{F}_o' =  \vec{F}_o\frac{1}{\tilde{\mu} U_s R_a}  =  \int_{S'} \vec{f}'(\vec{x}') \  \text{d}S'(\vec{x}')  
\label{eq_forcebal} 
\\  
 \vec{Q}_o' = \vec{Q}_o\frac{1}{\tilde{\mu} U_s R_a^2} =  \int_{S'} (\vec{x}' - \vec{x}'_{cm}) \times \vec{f}'(\vec{x}')  \ \text{d}S'(\vec{x}').  \label{eq_torquebal} 
\end{align}
...
&+  \frac{1}{8 \pi} \int_{S'}
 \vec{\psi'}(\vec{x}') \cdot  \bar{\bar{K'}}(\vec{x}',  \vec{x}'_s) 
 \cdot \hat{n}(\vec{x}) 
  \ \text{d}S' (\vec{x}') 
 \nonumber  \\
 &+ \vec{F'}_o \cdot \bar{\bar{G'}}(\vec{x}'_{s},\vec{x}'_{cm})
 +\frac{1}{8 \pi} \frac{\vec{Q'}_o \times
 (\vec{x}'_s   - \vec{x}'_{\text{cm}} ) }{\| \vec{x}'_s   - \vec{x}'_{\text{cm}} \|^3 },
  \label{eq_BI_DL_jump_noD}
 \end{align}
 for points on $S$. The non-dimensional force and torque, in this case, are
 \begin{eqnarray}
  \vec{F}_o' &=&  \int_{S'}  \vec{\psi}'( \vec{x}') \ \text{d}S(\vec{x}')  
 \label{alpha_dl_nonD} \\
  \vec{Q}_o' &=&  \int_{S'}  \vec{\psi}'(\vec{x}')  \cdot  (\bar{\bar{\bar{\epsilon}}} \cdot \vec{x}' )  \ \text{d}S(\vec{x}') ,
 \label{omega_dl_nonD}
 \end{eqnarray}
...
We found the analysis easier to perform when setting $\vec{U}'_a$ and $\vec{\Omega}'$ to known values, providing us with the velocity on the surface of the object via equation~(\ref{eq_solidbody_nonD}). We thus know 
 ...
 \end{equation}
 where the coefficients $\bar{\bar{\Pi}}'_{i,k}$ are constant. These constants may be computed analytically, and the corresponding formulas are given in Appendix~\ref{ap:chapter2}.
 ...
 \par
 For the double-layer potential, the unknown density, $\vec{\psi}'_k$, is found using the same discretization~(\ref{eq_discretized}) applied to equation~(\ref{eq_BI_DL_jump_noD}), resulting in a system with no rank deficiency. 
 See Appendix~\ref{ap:chapter2}.
 However, the values of the total force and torque terms must be solved simultaneously with the unknown density, resulting in a slightly larger system of size $3(N_f+2)\times 3(N_f+2)$. These additional  equations,~(\ref{alpha_dl_nonD}) and~(\ref{omega_dl_nonD}), are discretized as,
 \begin{eqnarray}
 0 \ &=& \
 \int_{S'}  \vec{\psi}'( \vec{x}')  \ \text{d}S'(\vec{x}') -  \vec{F}_o' \ = \
A' \sum_{k=1}^{N_f} \vec{\psi}'_k   -  \vec{F}_o', \label{eq_disc_dlF} \\
0 \ &=& \
  \int_{S'}  \vec{\psi}' ( \vec{x}') \cdot  (\bar{\bar{\bar{\epsilon}}} \cdot \vec{x}' )  \ \text{d}S'(\vec{x}')
 - \vec{Q}_o'
\ = \
 A' \sum_{k=1}^{N_f} \vec{\psi}'_k   \cdot \bar{\bar{\bar{\epsilon}}}  \cdot  \vec{x}'_k   - \vec{Q}_o' \label{eq_disc_dlQ}
 \end{eqnarray}
...
\begin{equation}
D' = -\vec{F}_o' \cdot \frac{\vec{U}_a}{||\vec{U}_a||} =  \vec{F}_o' \cdot \hat{k} =  \frac{\vec{F}_o\cdot \hat{k}}{ \tilde{\mu} U_s L}, 
\label{eq_drag}
\end{equation}
...
In the remainder of this chapter, we, therefore, present results obtained using the single-layer approach.
%-----------------
\subsection{Streamlines} 
\label{ch_streamlines}
\begin{figure}[ht]
	\begin{center}
		\epsfig{figure=figures/fig_streamlines_all.pdf, width=140mm}
	\end{center}
...
We then chose initial positions below the cube and used a second-order Runge-Kutta method to advance the positions in time using the corresponding velocities computed using equation~(\ref{eq_BI_sl_ND}) at each position. 
We compare the streamlines with four different resolutions of the cube; $\Delta x = 2,1, 0.5,$ and $0.25$. For the cube with $\Delta x = 2$, streamlines are seen to enter the interior of the cube.
\par
In Figure~\ref{fig_strlns} (a), we obtained the most inaccurate streamlines, violating our solid boundary surface assumption. This implies that we need more than one cube to model an aggregate. We observe the major error occurs at the corner of the cubes. 
We interpret the error at low resolution as smoothing of the cube. 
As we increase the resolution, the streamlines remain exterior to the cube.
...
\begin{equation}
T' = -\vec{Q}_o' \cdot \frac{\vec{\Omega}'}{||\vec{\Omega}' ||}  \text{\ \ \ and \ \ \ } \vec{Q}_o' = \frac{  \vec{Q}_o} {\tilde{\mu} ||\vec{\Omega}  || L^3 } 
\label{eq_Qprime}
\end{equation}
...
The gyration radius of a sphere of radius $R_s$ is $R_g = \sqrt{2/5} R_s$, and thus the Stokes hydrodynamic force acting on a sphere is $\vec{F}_o= 6 \pi \tilde{\mu} \vec{U}_a R_s \approx 29.8 \tilde{\mu} \vec{U}_a R_g$.  Also, the torque on a rotating sphere is $\vec{Q}_o= 8 \pi \tilde{\mu} \vec{\Omega} R_s^3 \approx 99.35 \tilde{\mu} \vec{\Omega} R_g^3$. For a sphere in an extensional flow, the exact solution of the straining force for the flow satisfying $\vec{U}_{bg} = \bar{\bar{M}}\cdot \vec{x}$ at infinity and $\vec{U}_{bg} = 0$ on the surface of the sphere is known~\cite{guazzelli_physical_2011}. The details of the computation of the corresponding straining force are given in the appendix. We find that for a matrix $\bar{\bar{M}}$ with unit eigenvector $\hat{v}_i$ and corresponding eigenvalue $\lambda_i$, the straining force is
\[
...
The depth of the fluid or shape of aggregates is typically considered to quantify the efficiency of transferring organic carbon via settling aggregates.
...
\par
To understand this application and track the concentration dynamics of a marine aggregate, we solve the three-dimensional advection-diffusion equation, that is 
\begin{equation}
	...
	\label{eq_ad_diff_3D}
\end{equation} 
where $C$ represents the CO$_2$ concentration. 
Note that another type of concentration can be considered depending on one's interest. 
We denote the diffusivity (m$^2$/s) of $C$ as $D_{C}$.
We may non-dimensionalize equation~(\ref{eq_ad_diff_3D}) using the same parameters as shown in~(\ref{eq_nonD}), in addition to the time parameter, $t' = \left(U_s/L \right) t$. 
As we non-dimensionalize equation~(\ref{eq_ad_diff_3D}), 
...
\label{eq_def_Pe}
 ...
 One can also find the Schmidt number to understand the diffusion rate of the fluid. Since we consider the Stokes flow, the Schmidt number is effectively infinite. 
\par With the Péclet number, the resulting advection-diffusion equation is
\begin{equation}
	\frac{\partial C' }{\partial t'} 
	= - \vec{u}' \cdot \nabla' C'
	+ \frac{1}{\text{Pe}} \nabla'^2 C'.
...
\end{equation}
Based on the the diffusivity of CO$_2$ in seawater \cite{park_performance_2020} measured, we find the Péclet number,
\begin{align}
	\text{Pe} 
...
\end{align}
Note that this could differ for sizes of an aggregate, and we will explore various Péclet numbers for our simulations within a reasonable range. 
\par 
...
\begin{align}
	...
	\label{eq_RK_step2}
\end{align}
where $C_j^n$  represents the discrete concentration value $C'$ at a numerical grid $x_j$ and $n-$th time step.
...
The finite differences method allows us to set various boundary conditions. We apply the central differences for both the advection and diffusion terms to obtain second-order convergence in space with finite differences. 
One concern is the stability of numerical methods. We, therefore, want to be careful about and keep track of the Courant-Friedrichs-Levy (CFL) condition for both the advection and diffusion terms,
\begin{equation}
...
\end{equation}
\par
On the other hand, the DFT is a well-known method to obtain spectral accuracy on a simple periodic domain with smooth data. It is a discretized version of the Fourier transform using the concept of Riemann sums in a bounded domain instead of infinite space. 
...
We note the computing time differences between the two methods. We can see this clearly in the right panel of Figure~\ref{fig_precision}. To get a particular error size, the finite differences scheme requires less time than the DFT method. The gap between the two methods becomes larger as we compute the solution with finer resolution.  
This is explained by the fact that the MATLAB built-in functions, \verb+fft+, and \verb+ifft+, spend more time than building the sparse matrix for the finite differences to compute derivatives. It shows that finite differences are more efficient for our situation. 
...
This can be done by changing variable from the velocities in the lab frame; we simply add $-\vec{U}_a$ to all velocities in the lab frame of reference,
\[
...
As a sample simulation, we use ten cubes to model an aggregate with a discontinuous initial concentration, zero throughout the fluid domain, outside of the aggregate, and a higher concentration inside and on the aggregate. Specifically, we prescribe values of $1$ for the inside of each cube and $0.5$ for all square faces. We also place $0.25$ and $0.125$ for all edges and corners of cubes, respectively. 
...
\begin{itemize}
	...
	\item  Final time $Nt = 500$ 
\end{itemize}
\end{framed}
For the sample simulation, we set $\text{Pe} = 100$.
This is in case it is necessary to observe a larger Péclet number that could have more numerical error. 
...
\par
We present snapshots of the simulation using the discontinuous initial concentration in Figure~\ref{fig_ic4_RK2_snap135}. It is a three-dimensional simulation; however, for clear visualization, we slice the domain at $x= -0.8$, which is the middle of $x-$axis.
...
After time $t \approx 30$ in Figure~\ref{fig_ic4_RK2_snap135}, we can see lower concentration is going outside of our numerical domain.  
 ...
 \end{center}
 \caption{Snapshots of concentration dynamics with a discontinuous initial concentration with finite differences in space and RK2 for time.}
...
\cite{panah_simulations_2017,camassa_retention_2013}. This process has already been modeled using spherical particles, but how diffusion and ambient stratification affect fractal-like aggregates remains unclear. By coupling the current simulations to a concentration field subject to advection-diffusion and tracking its effect on the fluid density within an aggregate, one should be able to obtain an improved approximation of the behavior of aggregates settling in a stratified ambient. 
Accounting for the effects of stratification on the flow itself requires the addition of a volume integral of the concentration to equation~(\ref{eq_slp}). This implies a significant increase in computational effort, but a careful implementation can be done that results in reasonable run-time. Such a derivation and implementation are the subject of the next chapter of this thesis. 
