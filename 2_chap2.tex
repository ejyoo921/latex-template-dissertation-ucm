...
\
 ...
 \
  ...
  \vec{\psi} = U_s \vec{\psi}',
%   \
%   \vec{F_o} = \mu U_s R_a \vec{F_o}',
%   \
%   \vec{Q_o} = \mu U_s R_a^2 \vec{Q_o}'.
\label{eq_nonD}
\end{equation}
...
\\
% Where is the condition number plot? and 
{\color{red}citation}
\\
...
\end{align}
% One can also find the pdf file of the seawater properties on \href{http://web.mit.edu/seawater/}{{\color{blue}Thermophysical properties of seawater}.}
...
%For our problem, we specifically want to obtain derivatives of a function using this Fourier space. In other words, we compute the derivatives of $\hat{f}_k$ first in the Fourier space and bring it back to physical space using inverse transform. 
...
% We test the two methods with a Gaussian function as the initial concentration and an advection velocity. Details are given in the next two sections. 

% This is close to the maximum Peclet number we may expect and may have large diffusion that is enough to use central difference scheme for advection term as well. 
%===SECTION 1.2=========================================
