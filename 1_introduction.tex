...
In particular, we explore four features of viscosity in complex fluids: 1) (shear) rate-dependent viscosity, 2) yield stress, 3) a second-order strain rate rheology, and 4) granular material flow.
...
Two types of rate-dependent viscosity, shear-thinning and thickening, are plotted with the green and purple lines, respectively.
...
\par
While the viscous stress tensor to model the rate-dependent and yield stress fluids is a multiple of their viscosity to the first-order strain rate, $\bm D$, other flow effects can be captured by a higher-order strain rate. In this thesis, we explore up to the second-order case and call them secondary flows.
...
They are highly ubiquitous and the second most common material on earth after water. 
