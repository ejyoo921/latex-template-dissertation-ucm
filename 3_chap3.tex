...
\begin{comment}
We can also prove this with simple arithmetics. 
...
\label{eq_F_by_S1}
\end{equation}
We notice that we can obtain the same integral value as equation (\ref{eq_F_by_S1}) on $S^6$, as well as  $S^2$ and $S^5$, in which their normals are $\hat{n}_2 = -\hat{n}_5 = (0,1,0)$. 
\end{comment}
...
the original authors of the FMM, have developed the methods and shared the source code. [{\color{blue} REFERENCES}] We choose to use their library, called \href{https://github.com/flatironinstitute/FMM3D}{{\color{blue}FMM3D}}. It provides the code of the $N-$body interactions governed by Laplace and Helmholtz equations in three-dimension.
...
% 	\]
{\color{blue} ADD MORE DESCRIPTION - FIND ANOTHER WRITING FILE.}
...
\label{eq_vol_part2}
\end{align}
can be computed by letting $\vec{v}^n$ in equation (\ref{eq_fmm3d_package}) as a product of a standard basis vector, $\hat{e}_j$, and the constant $x_3^n  \ {C}(x_3^n, t) $. We simply repeat the FMM3D for each component. 
\par
...
After implementing the FMM3D library into our program, we timed one more time to compare the fluid velocity computation shown as the green (or dashed line) in Figure \ref{fig_time_fmm_sum}. The efficiency of the computation increases about ten times. 
\begin{figure}[ht]
	\begin{center}
		% \includegraphics[scale=0.42]{./figures/fig_vel_mm5_t1}
		\includegraphics[scale=0.4]{./figures/fig_time_both_mm5_Nt10}
	\caption{CPU time with an aggregate made with 125 cubes for ten time steps. The blue line is the original homogeneous solution, and the orange represents the approximation using the FMM3D library.}
...
\clearpage
\section{Validation}
We examine the code by varying 1) time step size, 2) space step size, and 3) domain size. 
Due to the runtime of the entire simulation, we validate our code using an aggregate composed of ten cubes. 
From the center of mass of the aggregate,  $\vec{x}_{cm}$, we define our fluid domain, $V$, using the maximum radius of the aggregate, $R_m$, as 
\[
	V = [\vec{x}_{cm,1} - s R_m , \ \vec{x}_{cm,1} + s R_m ] \times
	[\vec{x}_{cm,2} - sR_m, \ \vec{x}_{cm,2} + s R_m]  \times 	
	[\vec{x}_{cm,3} - 2sR_m, \ \vec{x}_{cm,3} + 2sR_m ],
\]
where $s$ regulates the size of the fluid domain.


\begin{figure}[ht]
	\begin{center}
		\includegraphics[scale=0.3]{./figures/fig_NC10_snap_33}
		\includegraphics[scale=0.3]{./figures/fig_NC10_snap_65}
	\caption{Snapshots at time $t = 24$ and $t = 48$. We look at $y-z$ plane at $x = -0.3$, which is near the center of mass of the aggregate. }
	\label{fig_NC10_snaps}
\end{center}
\end{figure}
\begin{figure}[ht]
	\begin{center}
		\includegraphics[scale=0.25]{./figures/fig_NC10_dt_Ua3_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_dt_C_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_dt_cm3_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_dt_Fo3_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_dt_UaFo3_ratio}
...
\begin{figure}
	\begin{center}
		\includegraphics[scale=0.25]{./figures/fig_NC10_dt_diff_Ua3_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_dt_diff_C_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_dt_diff_cm3_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_dt_diff_Fo3_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_dt_diff_UaFo3_ratio}
	\caption{Absolute difference between two consecutive $\Delta t$ case. The orange line is obtained with $\Delta = 0.75$ and $\Delta = 0.5$, and we got the blue one with $\Delta = 0.75$ and $\Delta = 0.25$.}
	\label{fig_NC10_consec_diff}
\end{center}
\end{figure}
Domain size variation
\begin{figure}[h]
	\begin{center}
		\includegraphics[scale=0.25]{./figures/fig_NC10_s_Ua3_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_s_C_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_s_cm3_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_s_Fo3_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_s_UaFo3_ratio}
	\caption{Compare various domain sizes $\Delta s = 3, \ 4, \ 5$ in the step sizes $\Delta t = 0.5$ with $\Delta x = 1$.}
	\label{fig_NC10_compare_s}
\end{center}
\end{figure}
Space step variation.
\begin{figure}[ht]
	\begin{center}
		\includegraphics[scale=0.25]{./figures/fig_NC10_dx_Ua3_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_dx_C_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_dx_cm3_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_dx_Fo3_all}
		\includegraphics[scale=0.25]{./figures/fig_NC10_dx_UaFo3_ratio}
...
\end{center}
\end{figure}
Snapshots for $s = 3$ domain case.
\begin{figure}[ht]
	\begin{center}
		\includegraphics[scale=0.25]{./figures/fig_NC50_snap_1}
		\includegraphics[scale=0.25]{./figures/fig_NC50_snap_61}
		\includegraphics[scale=0.25]{./figures/fig_NC50_snap_121}
	\caption{Snapshots at time $t = 0, \  30, \ 60$. We look at $y-z$ plane at $x = -0.75$, which is near the center of mass of the aggregate. }
	\label{fig_NC50_snaps}
\end{center}
\end{figure}
...
\begin{figure}
	\begin{center}
		...
		\includegraphics[scale=0.25]{./figures/fig_NC50_s_UaFo3_ratio}
