...
We use AMReX framework, developed at LBNL, NREL, and ANL, to solve flow motion. We implement the granular material viscosity computation in \verb+incflo+, which is an Adaptive Mesh Refinement (AMReX)-based code for modeling the variable density incompressible Navier-Stokes equations without subcycling in time.
\par
...
\] 
where $\sigma_{xx} \neq \sigma_{yy} \neq \sigma_{zz}$.
% Once we know the values of $\eta_{i}$ and $\kappa_{j}$, we then update the velocity. 
%{\color{blue} How do we know these values?}
...
% {\color{blue} Are we keeping this the whole time? Is there any pressure change over time? Does $\phi$ come into play?}
