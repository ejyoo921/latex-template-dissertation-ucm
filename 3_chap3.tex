...
\begin{figure}[ht]
	...
	\end{center}
\end{figure}
We use ten cubes to form an aggregate model for the validation, as shown in Figure {\color{blue}ADD FIGURE}.
...
\par
As a sample result, Figure \ref{fig_NC10_snaps_all} shows three snapshots of the perturbation $C$, obtained with $\Delta t = 0.75, \ \Delta x = 1$, and $s = 5$, at $x = -0.3$. Note that the initial center of mass was $(-0.8, \    0.4, \      0)$. Since we use the moving frame of reference to compute the perturbation, the aggregate stays in the middle of the fluid domain in Figure \ref{fig_NC10_snaps_all}.
At a later time, we notice slight instabilities on the top of the fluid domain as the perturbation increases. Considering a large enough fluid domain compared to the aggregate size, we would like to observe the convergence of the quantities of interest near the aggregate. 
\subsection{Various time steps}
\begin{figure}[ht]
	\begin{center}
