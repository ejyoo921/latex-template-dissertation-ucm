...
Now, instead of constant density $\rho$, as in Chapter 2, we suppose the background fluid density varies linearly in the vertical direction,
\begin{equation}
...
\label{eq_density}
\end{equation}
where the non-zero constant $\alpha$ is based on the type of solute. 
\begin{figure}[ht]
	\begin{center}
...
\begin{equation}
	...
	\label{eq_Pd}
\end{equation}
To take the perturbation of the ambient fluid into account, we find the particular solution to the momentum equation (\ref{eq_extra_C}). Once we have one, simple addition to the homogenous solution would give us the entire solution due to the linearity of the system. 
\par
...
describes the effect coming from a singular force applied at $\vec{x} = \vec{y}.$
...
\label{eq_fund_p}
\end{equation}
where the kernel $\bar{\bar{G \ }}$ is the \textit{Stokeslet}, introduced in Chapter 1, euation (\ref{eq_stokeslet}).
...
\end{equation}
Note that the operator $\nabla$ is linear and with respect to $\vec{x}$. We are able to switch the order with the integral operator as follows,
\begin{equation}
	\tilde{\mu} \ \nabla^2 
	\int_{V}
	\biggl( \frac{\bar{\bar{G \ }}(\vec{x}, \vec{y})
...
\label{eq_vel_HP}
\end{equation}
As we see, the velocity at a point $\vec{u}$ is now dependent on space and time compared to the homogeneous fluid case. We can update the velocity field in time by coupling the solution (\ref{eq_vel_HP}) with the advection-diffusion equation (\ref{eq_ad_diff_nonD}) to model the solute perturbation, $C$.
 
%------------------------------------------------------------------
\subsection{Force balance}
\label{sec:force_balance}
\par
We can no longer impose the velocity of the aggregate, rather we need to solve for it.
This implies that the velocity of the aggregate, equation (\ref{eq_solidbody}),
\begin{equation}
	\vec{u}_s (\vec{x}, t)  = \vec{U}_a + \vec{\Omega} \times (\vec{x}- \vec{x}_{cm}),
	\nonumber
\end{equation}
is an unknown. Specifically, we now need to solve for the translational and angular velocity, $\vec{U}_a$ and $\vec{\Omega}$, respectively, by prescribing the total body force and total torque from the fluid on the aggregate.
...
\label{eq_Fo}
\end{equation}
To impose the total drag (\ref{eq_Fo}), we observe the right-hand side of the equation(\ref{eq_Fo}) which includes the gravity and buoyancy forces. The net force balance between all three types of force at equilibrium  is
\begin{equation}
...
To calculate the aggregate density, $\rho_a$, we should take its porosity, $\phi$, that is $ 0 \leq \phi \leq 1$.
...
We also set the porosity to $95\%$, or $\phi = 0.95$.
...
For the total torque, we use the form (\ref{eq_torquebal}), 
\begin{equation}
	\vec{Q}_o =\int_S \vec{f}\times (\vec{x} - \vec{x}_{cm}) \ \textrm{d}S =  \vec{0}.
\label{eq_Qo}
\end{equation}
As we wrote in the equation (\ref{eq_Qo}), we set zero total torque. Note that this does not imply that the aggregate is not rotating. 
%--------------------------------------------------
\subsection{Perturbation variation}
As the aggregate settles, the concentration and, therefore, the fluid density change,
\begin{equation}
...
\label{eq_AD_C}
\end{equation}
Note that the perturbation $C$ is dependent only on the gravitational direction ($z-$axis) since we assume the fluid density evolves on $z-$direction.
...
\end{equation}
\par
We first derive the dimensionless modified Stokes momentum equation,
\begin{equation}
	{\nabla'}^2  \vec{u}'(\vec{y})
	= \nabla {P_d}'(\vec{y}) \ - \  
...
\begin{figure}[ht]
	\begin{center}
		\includegraphics[scale=0.23]{./figures/fig_NC10_snaps_all.pdf}
...
\begin{figure}[ht]
	\begin{center}
		\includegraphics[scale=0.4]{./figures/fig_NC10_s_Ua3_all}
		\includegraphics[scale=0.4]{./figures/fig_NC10_s_cm3_all}
		\includegraphics[scale=0.4]{./figures/fig_NC10_s_Fo3_all}
		\includegraphics[scale=0.4]{./figures/fig_NC10_s_C_star}
	\caption{Compare various domain sizes with $ s = 3, \ 4, \ 5$ in the time step  $\Delta t = 0.5$ and space step $\Delta x = 1$. We show the settling speed (a), the position of the center of mass (b), the vertical force on the aggregate (c) and the value of the perturbation near the aggregate (d)}
...
\subsection{Various time step sizes}
...
\begin{figure}[ht]
	\begin{center}
		\includegraphics[scale=0.4]{./figures/fig_NC10_dt_Ua3_all}
		\includegraphics[scale=0.4]{./figures/fig_NC10_dt_cm3_all}
		\includegraphics[scale=0.4]{./figures/fig_NC10_dt_Fo3_all}
		\includegraphics[scale=0.4]{./figures/fig_NC10_dt_C_star}
...
\begin{figure}[h]
	\begin{center}
		\includegraphics[scale=0.4]{./figures/fig_NC10_dt_diff_Ua3_all}
		\includegraphics[scale=0.4]{./figures/fig_NC10_dt_diff_cm3_all}
		\includegraphics[scale=0.4]{./figures/fig_NC10_dt_diff_Fo3_all}
		\includegraphics[scale=0.4]{./figures/fig_NC10_dt_diff_C_star}
	\caption{Absolute differences between two lines from Figure \ref{fig_NC10_compare_dt}. }
	\label{fig_NC10_dt_diff_all}
\end{center}
\end{figure}

\subsection{Various space step sizes}
...
\begin{figure}[ht]
	\begin{center}
		\includegraphics[scale=0.4]{./figures/fig_NC10_dx_Ua3_all}
		\includegraphics[scale=0.4]{./figures/fig_NC10_dx_cm3_all}
		\includegraphics[scale=0.4]{./figures/fig_NC10_dx_Fo3_all}
...
\subsection{Various Péclet number}
\subsection{Various stratification strength}
