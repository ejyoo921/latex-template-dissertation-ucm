...
\label{fig_rheology}
\end{figure}
This broad class of fluids encompasses various materials of industrial and natural importance, such as granular fluids, polymeric fluids, and suspensions. 
% [{\color{blue}References for each of them}]
\par
Among these various features, we focus on the \textit{viscometric flow}; this type of flow is described by the viscosity $\mu$, and normal stress differences, called $N_1$ and $N_2$. To explain terminology, we write out $\bar{\bar{\sigma}}$ explicitly, 
\begin{equation}
  \bar{\bar{\sigma}} = 
  \begin{bmatrix}
...
\end{equation}
To describe a non-Newtonian flow behavior more accurately, the deviatoric stress tensor requires a higher-order relationship with the strain rate tensor $\bm{D}$. The following second-order complexity form of the deviatoric stress tensor is introduced by Rivlin-Ericksen tensors~\cite{rivlin_stress-deformation_1955},
\begin{equation}
   \boldsymbol{\tau} = 
   % 2 \nu_1  \bm{D} + 2 \nu_2  \bm{D}^2 ,
    \nu_0  \bm{A}_1 +  \nu_1  \bm{A}_1^2 + \nu_2 \bm{A}_2,
...
\par
With a constitutive rheology code, we can capture such complex continuum dynamics. 
...
\par
During the internship period in 2022, we focused on granular rheology, implementing a particular viscosity computation. Assuming a steady state and no external pressure changes, we solve an incompressible flow velocity using equations (\ref{eq_conserv_mass}) and (\ref{eq_momentum_NS}), it is necessary to evaluate the stress tensor, $\nabla \cdot \bar{\bar{\sigma}}$. Once we have the stress tensor of the form (\ref{eq_CN_tau}), which is non-linear in velocity, it becomes numerically complicated to solve the Navier-Stokes equations. The most convenient choice of time integration would be an explicit method. However, it may not be the best option, considering stability and efficiency. Focusing on its order of convergence, we implemented a two-stage Runge-Kutta method. We explain details in Chapter 4.

% In incflo, at the moment I was interning, there were three integrations were available: 1) Explicit Euler, 2) Crank-Nicolson, and 3) implicit Euler methods. 
